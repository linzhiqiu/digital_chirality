...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2):
        '''
            ...
            blur_width == 0 means no gaussian blur is applied.
        '''
        self.inplanes = 64
...
def get_resnet_model(resnet_name, pretrained=False, **kwargs):
    if resnet_name == 'resnet50':
        resnet_config = [3, 4, 6, 3]
    elif resnet_name == 'resnet101':
        resnet_config = [3, 4, 23, 3]

    model = ResNet(Bottleneck, resnet_config, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls[resnet_name]), strict=True)
    return model
